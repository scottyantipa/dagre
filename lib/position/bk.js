...
function balance(xss, align, layering, g) {
  ...
  const balanced = Object.fromEntries(
    Object.entries(originalBalanced).map(([nodeId, position]) => {
      return [nodeId, Math.round(position)];
    })
  );
...
// Find tree structures towards the end of the graph, eg leaf nodes
// in the structure are sink nodes in the graph. Then tighten up the nodes
// by bringing them closer together to their siblings if there are gaps.
function treeCompaction(g, layering, balanced) {
  ...
  // First we have to find the tree structures.
  // Start for now with just right tree structures (leaf nodes are sinks).
  // Then for each of these structures, tree to move the sibling nodes closer together
  // AND move their edges
  // NOTE: The dummy edge nodes will be a successor of the root node in the tree,
  // and a predecessor of the sink node.

  const sinkNodeIds = g.sinks();
  const candidateRootNodes = [];
  sinkNodeIds.forEach((sinkNodeId) => {
    const predecessors = g.predecessors(sinkNodeId);
    if (predecessors.length != 1) {
     return; 
    }
    const predecessorId = predecessors[0];
    const predecessorNode = g.node(predecessorId);
    if (!predecessorNode.dummy == "edge") {
     return; 
    }
    const predsOfPred = g.predecessors(predecessorId);
    if (predsOfPred.length != 1) {
     return; 
    }
    const predOfPred = predsOfPred[0];
    if (!candidateRootNodes.includes(predOfPred)) {
     candidateRootNodes.push(predOfPred);
    }
  })

  // Candidate root nodes is now an array of node ids which are one degree earlier
  // than at least some sink node. Now we need to see if these candidate root nodes a) form a tree
  // where they are the root, and b) the tree is only 2 levels dep, eg each child of the root node is a
  // sink of the dagre graph.
  //
  // This will be populated with the ids of the nodes which are
  // actual tree structures
  const treeRootNodeIds = [];
  candidateRootNodes.forEach((candidateId) => {
    let isTree = true;
    for (let childNodeId of g.successors(candidateId)) {
      if (candidateId == childNodeId) {
       isTree = false;
        break;
      }
      const childNode = g.node(childNodeId);
      if (!childNode.dummy == "edge") {
        isTree = false;
        break; 
      }
      const nextChildId = g.successors(childNodeId)[0];
      if (!nextChildId) {
        isTree = false;
        break;
      } else {
        // TODO: We shouldnt make it so all children of the root node
        // have to be a sink node, but we will only move sink nodes.
        const isSink = sinkNodeIds.includes(nextChildId);
        if (!isSink) {
          isTree = false;
         break; 
        }
    ...
    if (isTree) {
     treeRootNodeIds.push(candidateId) 
    }
  });
  
  // Now we have our tree structures, so iterate over them and adjust their children positions
  // See if there are any spaces between these siblings that
  // we could potentially remove
  treeRootNodeIds.forEach((rootNodeId) => {
    const childrenNodeIds = [];
    // This is a hash of child node id to the node id of the dummy edge that points the child to the root
    const edgeIdsByNodeId = {};
    // We already know the shape of the tree is an edge to a sink node
    g.successors(rootNodeId).forEach((successorId) => {
      const childNodeId = g.successors(successorId)[0];
      childrenNodeIds.push(childNodeId);
      edgeIdsByNodeId[childNodeId] = successorId;
    });
    const siblingPositions = childrenNodeIds
    ...
    // Now we know there is a gap, so fix it.
    ...
    const currentLayerIndex = layerIndexByNodeId[childrenNodeIds[0]];
    ...
    const rootNodePosition = balanced[rootNodeId];
    const siblingIdsSorted = childrenNodeIds.sort((aNodeId, bNodeId) => {
        return Math.abs(rootNodePosition - balanced[aNodeId]) - Math.abs(rootNodePosition - balanced[bNodeId])
    });

    // For each node try to walk it closer to the root node id
    siblingIdsSorted.forEach((siblingNodeId) => {
        recomputeLayerPositions();
        const currentPosition = balanced[siblingNodeId];
        if (currentPosition == rootNodePosition) {
            return;
        }
        const deltaToAdd = rootNodePosition > currentPosition ? 1 : -1;
        let bestPosition;
        while (true) {
            const nextPosition = (bestPosition != undefined) ? bestPosition + deltaToAdd : currentPosition + deltaToAdd;
            if (deltaToAdd == 1 && nextPosition >= rootNodePosition) {
                break;
            }
            if (deltaToAdd == -1 && nextPosition <= rootNodePosition) {
                break;
        ...
        if (bestPosition != undefined) {
            ...
            const edgeId = edgeIdsByNodeId[siblingNodeId];
            if (edgeId != undefined) {
                balanced[edgeId] = bestPosition;
            }
        }
    });
  })
...
function positionX(g) {
  ...
  treeCompaction(g, layering, balanced);
  return balanced;
}
